...
const PropertyModal = ({onClose, properties}) => {
  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur-md flex items-center 
      ...
      <div className='relative bg-white rounded-3xl max-w-xl w-full md:max-h-[90vh]
        ...
        <div className='relative h-[300px] md:h-[400px]'>
          ...
          <button onClick={onClose} className='absolute text-white top-10 right-5 -translate-y-1/2 
                                               bg-red-500 p-2 rounded-full cursor-pointer'>
            <FaX size={20}/>
          </button>
    
          <div className='absolute bottom-4 right-4 flex items-center gap-4'>
            <button onClick={() => setIsFavorite(!isFavorite)} className={`bg-white/80 p-2 rounded-full
