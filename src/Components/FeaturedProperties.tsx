...
const FeaturedProperties = ({setSelectedProperty}) => {
  return (
    <section className='max-w-auto py-16 px-4'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8'> 
        {properties.map((property) => (
          <div key={property.id} className="bg-white rounded-2xl drop-shadow-lg overflow-hidden
                                  hover:drop-shadow-xl hover:scale-105 transition-all
                                  duration-300 relative group cursor-pointer"
                onClick={()=> setSelectedProperty(property)}>

              <div className='relative'>
                <img src={property.image} className='w-full h-72 object-cover group-hover:scale-105
